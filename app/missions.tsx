    ...
    <MenuScreen title={`MISSION CONTACTS (${missionCount}) - CONTACTS (${NPCS.filter(npc => (!showUnlockedOnly || progression.level >= npc.unlockedAtRank) && (!filterFaction || npc.faction === filterFaction)).length})`}>
      ...
      <NeonBorder style={{ marginTop: 14 }}>
        ...
        <NeonBorder style={{ marginTop: 8 }} active>
          <Text style={{ fontFamily: terminalFont, color: terminalColors.cyan, fontSize: 12 }}>MISSION HISTORY</Text>
          {missionHistory.length === 0 ? (
            <Text style={{ fontFamily: terminalFont, color: terminalColors.muted, fontSize: 10, marginTop: 4 }}>No completed missions yet.</Text>
          ) : (
            missionHistory
              .slice()
